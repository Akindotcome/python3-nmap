...
def scan_techniques(nmt: nmap3.NmapScanTechniques, scan_type: str, target: str):
    if scan_type == "-sF":
...
def scan_discovery(
    nmd: nmap3.NmapHostDiscovery, scan_type: str, target: str, ports_num: int
):
    if scan_type == "-Pn":
...
def scan_command(nm: nmap3.Nmap, scan_type: str, target: str, domain: str):
    if scan_type == "-sA":
...
def launch(
    ...
    ports: typing.Optional[int] = None,
    templates: typing.Optional[int] = None,
):
    def tpl(i: int):
        template = {
    ...
    if templates or domain:
        if ports:
            ...
            choice = (tpl(21) or "") + str(ports)
            print("\n\nTrying option: ", choice)

            scan_type = tpl(21)
            if not scan_type:
                raise ValueError("No scan type provided")
            res = scan_discovery(nmd, scan_type, target, ports)
            ...
            if not scan_type:
                raise ValueError("No scan type provided")
            res = scan_command(nm, scan_type, "", domain)
            ...
            templates = typing.cast(int, templates)
            scan_type = tpl(templates)
            if not scan_type:
                raise ValueError("No scan type provided")
            print("\n\nTrying option: ", scan_type)

            if templates <= 7:
                res = scan_techniques(nmt, scan_type, target)
                ...
                res = scan_discovery(nmd, scan_type, target, 0)
                ...
                res = scan_command(nm, scan_type, target, "")
